// credit: joonkukang - https://github.com/junku901/dnn
...
RBM.prototype.train = function(settings) {
    var self = this;
    var lr=0.8, k= 1, epochs = 1500; // default learning rate, contrastive divergence k and epochs
    ...
    for(i=0;i<epochs;i++) {
        ...
        var deltaW = math.mulMatScalar(math.minusMat(math.mulMat(math.transpose(self.input),phMean), math.mulMat(math.transpose(nvSamples),nhMeans)), 1.0 / self.input.length);
...
RBM.prototype.propup = function(v) {
    ...
    return math.activateMat(preSigmoidActivation, math.sigmoid);
};
